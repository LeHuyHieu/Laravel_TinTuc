...
<div class="comment-form">
    ...
    <form class="form-contact comment_form" action="{{route($linkFormComment, $id)}}" method="post" id="commentForm">
        ...
        <input type="hidden" name="blog_id" id="blog_id" value="{{(isset($item->id))?$item->id:''}}">
        <input type="hidden" value="0" name="comment_id" id="comment_id" />
        <div class="row">
            <div class="col-12">
                <div class="form-group">
                    <textarea class="form-control w-100" name="message" {{(!Auth::check() || Auth::user()->email_verified_at == '')?'disabled':''}} id="message" cols="30" rows="9" placeholder="{{!Auth::check()?'Đăng nhập để viết bình luận':'Viết bình luận vào đây'}}"></textarea>
                </div>
            </div>
        </div>
        <div class="form-group">
            <button type="submit" id="btnSendMessage" class="button button-contactForm btn_1 boxed-btn" {{(!Auth::check() || Auth::user()->email_verified_at == '')?'disabled':''}}>Gửi tin nhắn</button>
        </div>
    </form>
</div>