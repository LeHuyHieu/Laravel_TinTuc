                ...
                <div class="card">
                    <div class="card-block">
                        <h4 class="card-title d-flex align-items-center">
                            <span class="d-inline-block">List youtube</span>
                            <form method="get" action="{{route('youtube.search')}}" class="d-inline-flex align-items-center ml-auto">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered">
                                ...
                                @foreach($youtube as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->tag}}</td>
                                        <td>
                                            {{($item->type_new == 0)?'Tin thường':''}}
                                            {{($item->type_new == 1)?'Tin hot':''}}
                                            {{($item->type_new == 2)?'Tin được tài trợ':''}}
                                            {{($item->type_new == 3)?'Tin youtube':''}}
                                        </td>
                                        <td>
                                            <form action="{{route('youtube.destroy', $item->id)}}" method="post">
                                                <a href="{{route('youtube.edit', $item->id)}}" class="btn btn-primary"><i class="mdi mdi-lead-pencil mr-1"></i> Edit</a>
                                                @csrf
                                                @method('DELETE')
                            ...
                            <div class="text-right">
                                {{$youtube->appends($_GET)->links('backend.paginations.admin')}}
                            </div>
                        </div>
                    </div>
                </div>
